...
#     KAPseudospectra.tune_trsm!(CUDABackend(), collect(CUDA.devices())[1])
...
function write_tune_profile(path::AbstractString, chosen; device::AbstractString = "")
    ...
    open(path, "w") do io
        println(io, "# KAPseudospectra tuning profile — generated by tune_trsm!, do not hand-edit")
