...
- `ihlpsa_adaptive.jl` — adaptive iteration-depth `ihlpsa`: σ contours alongside
  the per-grid-point Lanczos-depth maps, at two tolerances.
- `ihlpsa_adaptive_advanced.jl` — the intricate adaptive machinery: generalized
  pencils (B ≠ I, γ/δ weights), the convergence knobs (rtol/atol/nconfirm/
  nit_chunk/nit_max), reproducible seeds, the nit_max cap, `zpd` multi-batch, and
  multi-device fan-out via `devs`.
