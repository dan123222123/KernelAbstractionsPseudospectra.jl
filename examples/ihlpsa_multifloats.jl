...
HP = Complex{Float32x2}                  # double-single (~14 digits) — the accurate solve
...
println("  differ by >10% : ", count(>(0.1f0), D), " / ",
    length(D), "   ← single precision unreliable")
println("  max rel diff   : ", Float32(maximum(D)), "  at z = ", round(
    gx[w[2]] +
    im * gy[w[1]], digits = 3))
...
hi_adapt = solve(backend, HP; adaptive = true, rtol = 1.0f-12)
println("  adaptive vs fixed nit=12: max rel diff = ", Float32(maximum(abs.(hi_adapt .-
                                                                            hi) ./ hi)))
...
if true
    ...
    p1 = contourf(gx, gy, clamp.(log10.(Float32.(hi)), -17, 0);
        levels = collect(-17:0), color = :turbo,
        title = "log10 σ_min  ($HP)", xlabel = "Re z", ylabel = "Im z", aspect_ratio = 1)
    p2 = contourf(gx, gy, log10.(clamp.(Float32.(D), 1.0f-16, Inf32));
        levels = collect(-16:2), color = :inferno,
