...
using KernelAbstractionsPseudospectra, oneAPI, PrecompileTools
...
KernelAbstractionsPseudospectra.device(B::oneAPI.oneAPIBackend) = oneAPI.device()
KernelAbstractionsPseudospectra.device!(B::oneAPI.oneAPIBackend, dev) = oneAPI.device!(dev)
KernelAbstractionsPseudospectra.devices(B::oneAPI.oneAPIBackend) = oneAPI.devices()
KernelAbstractionsPseudospectra.get_bgarray(B::oneAPI.oneAPIBackend) = oneAPI.oneArray
# No Level Zero free-memory query; report free system RAM (shared with the iGPU).
KernelAbstractionsPseudospectra.device_bytes_available(B::oneAPI.oneAPIBackend) = (Sys.free_memory() |> Int)
# No memory-pool reclaim on oneAPI; fall back to GC.
KernelAbstractionsPseudospectra.device_reclaim(B::oneAPI.oneAPIBackend) = GC.gc()
...
KernelAbstractionsPseudospectra.warp_width(B::oneAPI.oneAPIBackend) = 32
function KernelAbstractionsPseudospectra.device_smem_bytes(B::oneAPI.oneAPIBackend)
...
function KernelAbstractionsPseudospectra.device_smem_per_sm(B::oneAPI.oneAPIBackend)
    Int(oneAPI.oneL0.compute_properties(oneAPI.device()).maxSharedLocalMemory)
end
...
# F32-only).
function KernelAbstractionsPseudospectra.supports_fp64(B::oneAPI.oneAPIBackend)
...
# oneAPI.
function KernelAbstractionsPseudospectra.warp_trsm_safe(::oneAPI.oneAPIBackend, wide::Bool)
    !wide &&
        KernelAbstractionsPseudospectra.intel_force_simd32() &&
...
KernelAbstractionsPseudospectra.tiled_gemm_safe(::oneAPI.oneAPIBackend, ::Type) = false
...
if oneAPI.functional()
    @setup_workload begin
        ...
        # `KernelAbstractionsPseudospectra.set_pdiv_accurate!(false)` so F32 solves don't emit `double`.
        Ts = KernelAbstractionsPseudospectra.supports_fp64(oneAPIBackend()) ? [ComplexF32, ComplexF64] :
             [ComplexF32]
        @compile_workload begin
            KernelAbstractionsPseudospectra._precompile_ihlpsa(oneAPIBackend(), collect(oneAPI.devices())[1], Ts)
        end
    end
end
