...
if CUDA.functional()
    @setup_workload begin
        @compile_workload begin
            # Default: precompile via the shuffle-free "column" solve. The warp/tiled kernels
            # are JIT-compiled on first use — historically their PTX didn't survive the
            # precompile→runtime boundary, and executing the shuffle kernels in the headless
            # precompile worker is the least-reliable spot for GPU execution.
