...
function KAPseudospectra.device_bytes_available(B::Metal.MetalBackend)
    dev = Metal.device()
    # Under memory pressure allocation can exceed the recommended max — floor at 0
    # so batch sizing never sees a negative budget.
...
function KAPseudospectra.device_smem_bytes(B::Metal.MetalBackend)
...
function KAPseudospectra.device_smem_per_sm(B::Metal.MetalBackend)
    Int(Metal.device().maxThreadgroupMemoryLength)
end
...
function KAPseudospectra.warp_trsm_safe(::Metal.MetalBackend, wide::Bool)
