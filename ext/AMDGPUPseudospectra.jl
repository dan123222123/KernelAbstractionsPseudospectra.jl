...
# Per-device queries for the trsm routing. warpSize is 64 on CDNA / 32 on RDNA; the register-warp
# solve adapts via R = cld(m, ws) (the tiled solve's panel width stays a fixed 32). NOTE: the
# 64-lane warp path is only exercised on AMD hardware (opt-in CI) — validate there.
