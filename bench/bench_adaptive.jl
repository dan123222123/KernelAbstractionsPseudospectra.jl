...
#
# Usage:
#   julia --project=bench bench/bench_adaptive.jl            # CPU, m ∈ {64,128,256}
#   julia --project=bench -e 'include("bench/bench_adaptive.jl"); bench(; ms=(64,128,256,512))'
...
function bench(
        backend = CPU(); ms = (64, 128, 256), region = ((-1, 3), (-3, 3)), gp = (64, 64),
    ...
    for m in ms
        ...
        local n_adp
        t_adp = @elapsed begin
            _, nit_grid = KAPseudospectra._ihlpsa_adaptive(
                backend, zg, P; nit_max = nit_fixed, devs)
