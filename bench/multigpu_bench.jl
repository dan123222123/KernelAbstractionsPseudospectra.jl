...
#############################################
# 3. CORRECTNESS  1-GPU vs ALL-GPU  (both drivers)
#############################################
function correctness()
    logln("\n## 3. CORRECTNESS  (1 GPU vs ", N_GPU, " GPUs)")
    cp = build(512)
    for (name, runner) in MODES
        r1 = runner(cp.zg, cp.P, ALLDEVS[1:1])
        rN = runner(cp.zg, cp.P, ALLDEVS)
        rel = maximum(abs.(r1 .- rN)) / maximum(abs.(r1))
        logln(@sprintf("  %-9s size=%s  relerr=%.3e  -> %s", name, string(size(r1)), rel, rel < 1e-5 ? "PASS" : "CHECK"))
    end
end
