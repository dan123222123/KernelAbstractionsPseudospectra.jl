...
#           julia --project=bench bench/multigpu_bench.jl
#
...
function build(n; box = (-2, 5, -4.5, 4.5))
...
function strong_scaling()
    ...
    for (name, runner) in MODES
        ...
        logln(@sprintf("  %-6s %-11s %-13s %-9s %-10s",
            "ndev", "time(s)", "Mgridpts/s", "speedup", "eff"))
        t1 = NaN
        for ndev in 1:N_GPU
            devs = ALLDEVS[1:ndev]
            ...
            ndev == 1 && (t1 = t)
            gpps, spd = (G*G)/t, t1/t
            logln(@sprintf("  [%s] %-3d  %-11.4f %-13.3f %-9.2f %-6.1f%%",
                clock(), ndev, t, gpps/1e6, spd, 100*spd/ndev))
            println(ss_csv, @sprintf("%s,%d,%.6f,%.3f,%.4f,%.4f", name, ndev, t, gpps, spd,
                spd/ndev))
            flush(ss_csv)
        end
    end
...
function size_sweep()
    ...
    logln(@sprintf("  %-7s %-9s %-11s %-11s %-13s", "n", "mode", "ihlpsa(s)", "pencil(s)",
        "Mgridpts/s"))
    for n in (512, 1024, 2048, 4096)
        ...
        for (name, runner) in MODES
            ...
            logln(@sprintf("  [%s] %-7d %-9s %-11.4f %-11.4f %-13.3f",
                clock(), n, name, t, ts, (G*G)/t/1e6))
            println(sz_csv, @sprintf("%d,%s,%.6f,%.6f,%.3f", n, name, t, ts, (G*G)/t))
            flush(sz_csv)
        end
    end
    close(sz_csv)
